...
class Main:
    def __init__(self):
        ...
        if REASON.upper() in ('1', 'ILLEGAL CONTENT'):
            ...
            print('\n[!] Reason invalid.')
            os.system(
                'title [Xmas Mass Reporter] - Restart required &&'
                'pause >NUL &&'
                'title [Xmas Mass Reporter] - Exiting...'
            )
            time.sleep(3)
            os._exit(0)
    ...
    def _update_title(self):
        while True:
            os.system(f'title [Xmas-Mass-Reporter] - Sent: {self.sent} ^| Errors: {self.errors}')
            time.sleep(0.1)
...
if __name__ == '__main__':
    os.system('cls && title [Xmas Mass Reporter] - Main Menu')
    main = Main()
    main.setup()
